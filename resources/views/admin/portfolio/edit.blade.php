...
@section('title', 'Editar: ' . $project->title . ' | REW CRM')
...
<div class="crm-header">
    <div>
        ...
        <h1 class="crm-title">Editar Proyecto: {{ $project->title }}</h1>
        <p class="crm-subtitle">
            Modifica la narrativa técnica, actualiza la captura WebP o re-analiza el sitio web en vivo.
        </p>
    </div>
    <div>
        <a href="{{ route('portafolio.show', $project->slug) }}" target="_blank" class="crm-btn crm-btn-secondary">
            <span>👁️</span> Ver en Portafolio Público ↗
        </a>
    </div>
</div>

<!-- 1. Smart URL Analyzer & Re-Capture Tool -->
<div class="crm-card-panel" style="margin-bottom: 2rem; border-left: 4px solid var(--crm-primary); background: linear-gradient(180deg, #ffffff 0%, #f8fafc 100%);">
    <div style="display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 1rem; margin-bottom: 1rem;">
        <div>
            <h3 style="font-size: 1.15rem; font-weight: 800; color: #0f172a; margin: 0; display: flex; align-items: center; gap: 8px;">
                🚀 Re-Analizador de Página & Actualizador de Captura WebP
            </h3>
            <p style="font-size: 0.85rem; color: var(--crm-text-muted); margin: 4px 0 0 0;">
                ¿El sitio del cliente cambió? Ejecuta una nueva captura completa al 100% para reemplazar la captura anterior y refrescar tecnologías.
            </p>
        </div>
    ...
    <div style="display: flex; gap: 0.75rem; align-items: center; flex-wrap: wrap;">
        <div style="flex: 1; min-width: 280px; position: relative;">
            <input type="url" id="analyzerUrlInput" value="{{ $project->project_url }}" placeholder="https://ejemplo.cl" 
        ...
        <button type="button" id="startAnalyzerBtn" class="crm-btn crm-btn-primary" style="padding: 0.75rem 1.25rem; font-weight: 800; white-space: nowrap;">
            <span>🔍</span> Re-Capturar y Actualizar WebP
        </button>
    </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 1.5rem;">
            ...
            <div class="crm-card-panel">
                ...
                <div class="crm-form-group">
                    ...
                    <input type="text" name="title" id="projectTitle" value="{{ old('title', $project->title) }}" required 
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                    <div class="crm-form-group">
                        ...
                        <input type="text" name="slug" id="projectSlug" value="{{ old('slug', $project->slug) }}" required 
                    ...
                    <div class="crm-form-group">
                        ...
                        <input type="text" name="client" id="projectClient" value="{{ old('client', $project->client) }}" required 
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                    <div class="crm-form-group">
                        ...
                        <input type="text" name="category" id="projectCategory" list="categoriesList" value="{{ old('category', $project->category) }}" required 
                    ...
                    <div class="crm-form-group">
                        ...
                        <select name="status" id="projectStatus" class="crm-form-control">
                            ...
                            <option value="En Desarrollo" {{ old('status', $project->status) === 'En Desarrollo' ? 'selected' : '' }}>En Desarrollo</option>
                            <option value="En Pausa" {{ old('status', $project->status) === 'En Pausa' ? 'selected' : '' }}>En Pausa</option>
                        </select>
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;">
                    <div class="crm-form-group">
                        ...
                        <input type="url" name="project_url" id="projectUrl" value="{{ old('project_url', $project->project_url) }}" 
                    ...
                    <div class="crm-form-group">
                        ...
                        <input type="date" name="project_date" id="projectDate" value="{{ old('project_date', $project->project_date) }}" 
            ...
            <div class="crm-card-panel">
                ...
                <div class="crm-form-group">
                    ...
                    <input type="text" name="role" id="projectRole" value="{{ old('role', $project->role) }}" required 
                ...
                <div class="crm-form-group">
                    ...
                    <input type="text" name="technologies" id="projectTechs" value="{{ old('technologies', $project->technologies) }}" required 
                ...
                <div class="crm-form-group">
                    ...
                    <textarea name="summary" id="projectSummary" rows="3" required 
                              class="crm-form-control @error('summary') is-invalid @enderror">{{ old('summary', $project->summary) }}</textarea>
                ...
                <div class="crm-form-group">
                    ...
                    <textarea name="full_description" id="projectDescription" rows="10" 
                              style="font-family: inherit; line-height: 1.6;"
                              class="crm-form-control @error('full_description') is-invalid @enderror">{{ old('full_description', $project->full_description) }}</textarea>
            ...
            <div class="crm-card-panel">
                ...
                <div id="resultsContainer" style="display: flex; flex-direction: column; gap: 0.75rem;">
                    @php
                        $resultsList = old('results', is_array($project->results) ? $project->results : []);
                        if (empty($resultsList)) {
                            $resultsList = ['Optimización integral lograda'];
                        }
                    @endphp
                    @foreach($resultsList as $res)
                        <div class="result-row" style="display: flex; gap: 0.5rem; align-items: center;">
                            ...
                            <input type="text" name="results[]" value="{{ $res }}" 
            ...
            <div class="crm-card-panel">
                ...
                <div class="crm-form-group">
                    ...
                    <input type="text" name="meta_title" id="projectMetaTitle" value="{{ old('meta_title', $project->meta_title) }}" 
                           class="crm-form-control">
                </div>
                ...
                <div class="crm-form-group">
                    ...
                    <textarea name="meta_description" id="projectMetaDescription" rows="2" 
                              class="crm-form-control">{{ old('meta_description', $project->meta_description) }}</textarea>
                </div>
            </div>
        ...
        <div style="display: flex; flex-direction: column; gap: 1.5rem; position: sticky; top: 90px;">
            ...
            <div class="crm-card-panel">
                ...
                <div style="margin-bottom: 1.25rem;">
                    <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; font-weight: 700; font-size: 0.92rem; color: #0f172a;">
                        <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $project->is_featured) ? 'checked' : '' }} style="width: 18px; height: 18px; accent-color: var(--crm-primary);">
                ...
                <div class="crm-form-group" style="margin-bottom: 1.5rem;">
                    ...
                    <input type="number" name="order" value="{{ old('order', $project->order) }}" class="crm-form-control" style="width: 100px;">
            ...
            <div class="crm-card-panel">
                ...
                <div id="imagePreviewContainer" style="width: 100%; height: 260px; border-radius: 10px; overflow: hidden; background: #0f172a; border: 2px solid var(--crm-border); margin-bottom: 1rem; position: relative; display: flex; align-items: center; justify-content: center;">
                    <img id="imagePreviewEl" src="{{ old('featured_image', $project->featured_image) }}" 
                         alt="{{ $project->title }}"
                ...
                <div class="crm-form-group">
                    ...
                    <input type="text" name="featured_image" id="projectFeaturedImage" 
                           ...
                           class="crm-form-control">
                    <span class="crm-form-help">Ruta interna (ej: /images/portfolio/{{ $project->slug }}.webp).</span>
                </div>

                <div class="crm-form-group" style="margin-bottom: 0;">
                    <label class="crm-form-label">O Reemplazar con Archivo</label>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    startBtn.addEventListener('click', function() {
        ...
        startBtn.innerHTML = '<span>⏳</span> Analizando & Re-Capturando...';
        statusBox.style.display = 'block';
        statusBox.style.background = '#eef2ff';
        statusBox.style.borderColor = '#c7d2fe';
        statusBox.style.color = '#3730a3';
        statusMsg.textContent = 'Iniciando navegador headless. Desplazando página completa para forzar lazy-load y capturar en alta resolución...';
        ...
        .then(function(data) {
            ...
            startBtn.innerHTML = '<span>🔍</span> Re-Capturar y Actualizar WebP';
            ...
            if (data.technologies && !document.getElementById('projectTechs').value) {
    ...
    addResultBtn.addEventListener('click', function() {
        ...
        row.innerHTML = `
            <span style="color: #059669; font-weight: 800; font-size: 1rem;">✓</span>
            <input type="text" name="results[]" value="" placeholder="Ej: Reducción del tiempo de carga" 
                   class="crm-form-control" style="font-size: 0.88rem;">
            <button type="button" class="remove-row-btn crm-btn-icon delete-btn" title="Eliminar este resultado">
                ✕
            </button>
